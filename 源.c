...
{
	FILE* fphzk = NULL;
	int i,j,k,l,m,p,n,offset;
	int flag,check,temp_ASCII;
	int len,num,len_word,tag=0;
	char temp_; 
	...
	while (1) 
		...
		{   printf("输入要生成字模的英文或标点(多个):");
			scanf("%s",&word);
			if (*word == '\n')
				break;
				len_word=strlen((char*)word); 
				for(i=0;i<=len_word;i++)
				{
					temp_ASCII=word[i]-32;
					for(j=0;j<=15;j++)
					{
						temp[j]=str_ASCII[temp_ASCII][j];
					}
					move(temp);
				}
		/*for(j=0;j<len_word;j++);
			{
				//temp_=word[j];
				//printf("%c",temp_);
				temp_ASCII=word[j]-32;
				for (k = 0; k < 16; k++) 
				{
						itoa(str_ASCII[temp_ASCII][k],temp,2);
						len=strlen(temp)-1;
						for(m=0;m<=7;m++)
						{
							Out_put[m]=0;
			...
			printf("uchar code key[16] = {");
			for (p= 0; p < 15; p++) 
			 {
				printf("0x%02X,", str_ASCII[(word[j]-1)][k]);
			 }
			printf("\n");
			}*/
			}	
